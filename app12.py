...
# from models import db, Project, Testimonial, Offer
...
#         return render_template('index.html', projects=projects, testimonials=testimonials, offers=offers)
...
#     # Admin-style uploader page (simple, not secured)
...
#         if pform.validate_on_submit() and pform.submit.data and 'image' in request.files:
#             f = request.files['image']
#             filename = None
#             if f and allowed_file(f.filename):
#                 filename = secure_filename(f.filename)
#                 f.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
#             proj = Project(title=pform.title.data, description=pform.description.data, image_filename=filename)
#             db.session.add(proj)
#             db.session.commit()
#             flash('Project uploaded')
#             return redirect(url_for('admin'))

#         if tform.validate_on_submit() and tform.submit.data and 'photo' in request.files:
#             f = request.files['photo']
#             filename = None
#             if f and allowed_file(f.filename):
#                 filename = secure_filename(f.filename)
#                 f.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
#             t = Testimonial(customer_name=tform.customer_name.data, content=tform.content.data, photo_filename=filename)
#             db.session.add(t)
#             db.session.commit()
#             flash('Testimonial uploaded')
#             return redirect(url_for('admin'))

#         if oform.validate_on_submit() and oform.submit.data:
#             o = Offer(title=oform.title.data, details=oform.details.data, active=True)
#             db.session.add(o)
#             db.session.commit()
#             flash('Offer saved')
#             return redirect(url_for('admin'))

#         return render_template('admin.html', pform=pform, tform=tform, oform=oform)

#     @app.route('/uploads/<filename>')
#     def uploaded_file(filename):
#         return send_from_directory(app.config['UPLOAD_FOLDER'], filename)

#     # Small JSON endpoint to check offers (useful for showing modal once per visitor)
...
from config import Config
from models import db, Project, Testimonial, Offer, AdminUser
from models import db, Project, Testimonial, Offer, Consultation
...
def create_app():
    ...
    with app.app_context():
        ...
        if not existing_admin:
            ...
            db.session.commit()
            app.logger.warning(f'No admin found — created default admin "{username}". PLEASE change the password (ADMIN_PASS) for production.')

    @app.context_processor
    def inject_now():
        # handy for base.html footer
    ...
    def projects_page():
        projects = Project.query.order_by(Project.created_at.desc()).all()
        return render_template('projects.html', projects=projects)
    # inside create_app() in app.py (replace existing /api/active-offers route)
    ...
    # Admin-style uploader page (secured)
    @app.route('/admin', methods=['GET', 'POST'])
    @login_required
    def admin():
        pform = ProjectForm()
    ...
    # @login_required  # keep this if you already use login protection
    ...
    def book_consultation():
        """Handle 'Book Consultation' / contact form submissions."""
